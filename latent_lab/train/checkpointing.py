...
Responsibilities:
  ...
  * Identity-bound adapter bundles v2 — best_params.pt carries
    (model_id, pinned revision, recipe, suite hash) plus a sha256 content
    digest over every tensor's exact bytes; loads prevalidate version,
    kind, identity, recipe, digest, keys/tensor/shape/dtype/finiteness
    before anything is returned, so failed loads are atomic.
  * guarded_optimizer_step — cheap fail-stop stepping: no optimizer.step()
    ...
    after the update. ANY clip/step/post-check failure raises one explicit
    fatal run-invalid exception; the caller must terminate the run and is
    never allowed to emit success artifacts from the mutated in-memory
    state. Recovery happens exclusively from the last atomically committed
  ...
  * Durable artifacts: atomic temp+fsync+replace JSON writing, file
    digests, explicit run-status semantics so interrupted/fatal runs can
    never be confused with complete evidence.
"""
