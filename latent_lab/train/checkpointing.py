...
class _ParamSnapshot:
    ...
    index, and each slot keeps the collision-safe capture-time storage
    key. Two distinct Parameters sharing one storage therefore share one
    slot, so rollback reinstates their aliasing bit-exactly. Parameter
    objects are NEVER copied or replaced — identity is preserved by
    construction."""
